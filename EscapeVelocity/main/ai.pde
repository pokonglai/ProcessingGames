...
public class AIDirector
{
  ...
  public AIDirector(float d)
  {
    ...
    max_y = height - Global.HUD_HEIGHT - Global.WORLD_WALL_EDGE_OFFSET;
  }
  ...
  void buildGraph(Floor f)
  {
    ...
    // next eliminate all the nodes that are "too close" to the physical objects
    ...
    for (GraphNode n : graph.getNodeArray())
    {
      ...
      if (floor.spaceship.outer.contains(n.xf(), n.yf())) badNodes.add(n.id());
      for (ResourceDeposit res : floor.deposits)
      {
        if (res.outer.contains(n.xf(), n.yf()))
          badNodes.add(n.id());
      }
    }
  ...
  GraphNode[] randomPath(float max_len)
  {
    GraphNode s = randomNode();
    GraphNode e = randomNode();
    pathfinder.search(s.id(), e.id());
  ...
  void displayGraph(PGraphics g)
  {
    g.fill(0, 0, 0);
    g.stroke(0);
    g.strokeWeight(1);
    for (GraphEdge e : graph.getAllEdgeArray()) g.line(e.from().xf(), e.from().yf(), e.to().xf(), e.to().yf());
    g.noStroke();
    for (GraphNode n : graph.getNodeArray()) g.ellipse(n.xf(), n.yf(), 3, 3);
  }
}
